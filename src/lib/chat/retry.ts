...
function getRetryDelayMs(attempt: number, retryAfterHeader?: string | null): number {
  ...
  return INITIAL_CHAT_RETRY_DELAY_MS * 2 ** (attempt - 1);
}

export async function withChatRetry<T>(operation: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 1; attempt <= MAX_CHAT_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (error instanceof AppError) {
        if (!isRetryableStatus(error.statusCode) || attempt === MAX_CHAT_ATTEMPTS) {
          throw error;
        }
